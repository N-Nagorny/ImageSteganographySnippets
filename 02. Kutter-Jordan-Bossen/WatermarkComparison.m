...
for i = 1:image1_size(1)
    for j = 1:image1_size(2)
        MSE = MSE + double((image1(i, j) - image2(i, j))^2);
        num = num + double(image1(i, j)^2);
        denom = denom + double((image1(i, j) - image2(i, j))^2);
    end
    MSE_blocks(k) = MSE;
